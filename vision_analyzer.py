...
VISION_MODEL    = os.environ.get("VLM_MODEL", "llama3.2-vision:11b")
...
VLM_MAX_RETRIES = 2   # analyze_now() の最大リトライ回数
...
class VisionAnalyzer:
    ...
    def __init__(
        ...
        self.monitor_index = monitor_index  # 1=プライマリ、2以降=サブモニター
        self._latest_context: str = ""
        self._consecutive_failures: int = 0  # 連続失敗カウンター
    ...
    def analyze_now(self, prompt: str = VISION_PROMPT) -> Optional[str]:
        ...
        VLM 呼び出しが失敗した場合は VLM_MAX_RETRIES 回リトライする。
        それでも失敗した場合はキャッシュされた直前の結果を返す（Noneよりも有用）。
        呼び出し元が None を受け取ったときの挙動はフォールバック話題遷移に移譲する。

        Returns:
            VLMの応答テキスト、キャッシュ、または None（キャプチャ自体が失敗した場合）
        """
        ...
        for attempt in range(1, VLM_MAX_RETRIES + 1):
            ...
            if attempt < VLM_MAX_RETRIES:
                logger.debug("VLM attempt %d/%d failed, retrying...", attempt, VLM_MAX_RETRIES)
        ...
        if result:
            ...
            with self._lock:
                ...
                cached = self._latest_context
            if self._consecutive_failures >= 3:
                logger.warning(
                    "VLM failed %d times in a row; topic fallback recommended",
                    self._consecutive_failures,
                )
            # キャッシュが残っていれば返す（沈黙よりもマシ）
            result = cached if cached else None

        return result

    def should_use_topic_fallback(self) -> bool:
        """VLM 連続失敗が多く、話題遷移フォールバックを推奨するか"""
        with self._lock:
            return self._consecutive_failures >= 3

    @staticmethod
    def get_fallback_topic() -> str:
        """VLM 失敗時のフォールバック話題をランダムに返す"""
        import random
    ...
    def list_monitors() -> list[dict]:
        """利用可能なモニターの一覧を返す（index=0 は全モニター合成）"""
        if not _MSS_AVAILABLE:
            return []
        try:
            with mss.mss() as sct:
    ...
    def _capture_screenshot(self) -> Optional[str]:
        """
        指定モニター（self.monitor_index）をキャプチャし JPEG base64 で返す。
        ...
        try:
            with mss.mss() as sct:
                ...
                monitor = monitors[idx]  # 指定モニター（デフォルト: プライマリ）
    ...
    def _call_vision(self, image_b64: str, prompt: str = VISION_PROMPT) -> Optional[str]:
        ...
        try:
            ...
            res = httpx.post(OLLAMA_API_URL, json=payload, timeout=VISION_TIMEOUT)
